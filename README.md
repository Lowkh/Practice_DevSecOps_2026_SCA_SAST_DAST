...
- SAST with CodeQL (source code)
- SCA with OWASP Dependency-Check (dependencies)
- DAST with OWASP ZAP (running app, non‑blocking)

You will copy–paste files, push to GitHub, and then read / fix security findings.  
The DAST job is configured to **stay green** even when ZAP finds alerts.
...
(Old on purpose so SCA finds issues.)
...
### 4.3 `3-dast-only.yml` — DAST (ZAP, non‑blocking)
...
3. Download artifacts:
   ...
   - SCA: `sca-reports`
   - DAST: `zap-baseline-reports`, `zap-fullscan-reports`

4. Open the **Security** tab to see SAST and SCA results in GitHub’s UI.
